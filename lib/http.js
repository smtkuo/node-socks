	...
	var sockserver = net.createServer(function(client) {
	  ...
	  function relay_connection(req) { //console.log(req)
		...
		if (req.method != 'CONNECT') {
		    ...
		    buffer = buffer_add(Buffer.from(header, 'utf8'), buffer.slice(_body_pos));	
...
function proxyReady() {
  ...
  if (this.req.method == 'CONNECT')
	this.write(Buffer.from("HTTP/1.1 200 Connection established\r\nConnection: close\r\n\r\n"));				  
  else {  
	  // creating response
...
function buffer_add(buf1, buf2) {
  var re = Buffer.alloc(buf1.length + buf2.length);
  buf1.copy(re);
